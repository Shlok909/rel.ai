...
export async function POST(req: Request) {
  try {
    ...
    // 4. Mark mat_guide_used for free preview tracking
    await supabase
      .from("profiles")
      .update({ mat_guide_used: true } as any)
      .eq("id", user.id);

    // 5. Consume Credit & Save History on success
    const uiName = template.META?.uiName || "Maturity Guide";
    const summaryText = answers.comm 
      ? (answers.comm.length > 50 ? answers.comm.substring(0, 50) + "..." : answers.comm)
      : "Self-reflection maturity assessment";
    ...
    console.error("API error in maturity-guide:", error);
    return NextResponse.json(
