...
export default async function RelationshipIssuesPage() {
  ...
  return (
    <AppShell>
      <TopBar title={moduleData?.uiName || "Relationship Issues"} credits={creditsData?.credits_remaining ?? 0} showBack />
        ...
        <div className="flex flex-col gap-3">
          {moduleFeatures.map((fid) => {
            ...
            const badgeLabel = accessState === "coming_soon" ? "COMING SOON" : 
                              f.isFreePreview ? "1-TIME FREE USE" : 
                              f.access[planId as keyof typeof f.access] === "full" ? "FREE" : "BOOST";

            const isUsedPreview = f.isFreePreview && (profile as any)?.mat_guide_used;

            const ctaLabel = isLocked ? "Join the Waitlist" : 
                            isUsedPreview ? "Limit Reached" :
